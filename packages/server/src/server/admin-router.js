...
import { ROLES, AUDIT_ACTION, AUDIT_RESOURCE, AUDIT_DECISION } from '@presence/core/core/policies.js'
...
const createAdminRouterR = Reader.asks(({ userContextManager, presenceDir, logger, auditWriter }) => {
  ...
  // closure 내부 정의 — refactor.md "모듈 스코프 최소화" 준수.
  ...
  const requireAdmin = (req, res, next) => {
    const isAdminByRole = Array.isArray(req.user?.roles) && req.user.roles.includes(ROLES.ADMIN)
  ...
  router.post('/policy/reload', async (req, res) => {
    ...
    try {
      const baseEntry = {
        ...
        caller,
        action: AUDIT_ACTION.POLICY_RELOAD,
        resource: AUDIT_RESOURCE.CEDAR_EVALUATOR,
        matchedPolicies: [],
      }
      if (outcome.kind === 'success') {
        const r = outcome.result
        auditWriter.append({
          ...baseEntry,
          decision: AUDIT_DECISION.SUCCESS, errors: [],
          reloadedAt: r.reloadedAt, reloadStartedAt: r.reloadStartedAt,
        })
      } else {
        auditWriter.append({
          ...baseEntry,
          decision: AUDIT_DECISION.FAIL, errors: [outcome.err.message],
...
export { createAdminRouter, createAdminRouterR }
