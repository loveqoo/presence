...
const unwrap = (m) => Maybe.fold(() => { throw new Error('unexpected Nothing') }, x => x, m)
...
async function run() {
  ...
  // E3. 정상 분리
  {
    ...
    assert(split.remaining.length === 5, 'extractForCompaction: remaining count')
  }
  ...
  // S1. format 검증
  {
    ...
    assert(typeof entry.ts === 'number', 'summaryEntry: ts is number')
  }
