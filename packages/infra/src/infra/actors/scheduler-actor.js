...
class SchedulerActor extends ActorWrapper {
  ...
  constructor(store, onDispatch, opts = {}) {
    const { logger, pollIntervalMs = 60_000 } = opts
  ...
  recordSuccess(runId, jobId, result) {
    try {
      ...
      this.warn('job_done: finishRun failed', { error: e.message })
    }
  }
  ...
  handleJobFailure(msg) {
    ...
    try {
      ...
      this.warn('job_fail: handler error', { error: e.message })
    }
  }
  ...
  scheduleRetry(job, attempt) {
    ...
    this.info(`Job retry scheduled: ${job.name} (attempt ${attempt + 1}/${job.maxRetries})`)
  }
  ...
  disableJob(job, attempt) {
    ...
    this.warn(`Job disabled after ${attempt} failures: ${job.name}`)
  }
  ...
  runCleanup() {
    try {
      ...
      if (removed > 0) this.info(`Scheduler cleanup: removed ${removed} expired runs`)
  ...
  warn(msg, meta) { (this.logger || console).warn(msg, meta) }
}
