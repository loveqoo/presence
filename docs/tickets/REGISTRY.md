...
| FP-65  | open     | medium   | infra| 경로 차단 에러 메시지가 존재하지 않는 tools.allowedDirs 설정 안내       | docs/ux/tui-status-tools.md         |
| FP-66  | open     | low      | tui  | StatusBar dir 세그먼트가 TUI cwd 표시 — 실제 워크스페이스와 불일치      | docs/ux/tui-status-tools.md         |
...
| KG-18  | open     | medium | spec   | 5진입점 canAccessAgent 테스트 정적 grep 수준 — 동적 spy 강화 필요                | docs/specs/agent-identity.md#KG-18 |

## 통계

- FP 총 **66개** — open **2**, resolved **64**
- KG 총 **18개** — open **4**, resolved **14**
- Severity 분포 (open만): high 1 (KG-17) · medium 3 (KG-15, KG-16, KG-18)
