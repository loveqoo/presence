...
| KG-15  | open     | medium | server | Admin singleton session 강제 미구현 (concurrent approve race 대응)              | docs/specs/agent-identity.md#KG-15 |
| KG-16  | open     | medium | server | M3 primaryAgentId 미이행 — `{username}/default` hardcode                        | docs/specs/agent-identity.md#KG-16 |
| KG-17  | open     | high   | server | A2A JWT 서명 검증 stub (`X-Presence-Caller` 헤더) — authz phase 대기            | docs/specs/agent-identity.md#KG-17 |
...
| KG-20  | open     | medium | spec   | AgentId branded type 런타임 강제 부재 — validateAgentId 우회 회귀 검증 없음       | docs/specs/agent-identity.md#KG-20 |
| KG-21  | open     | medium | spec   | Parser→Resolver→Authz 순서 런타임 검증 부재 — resolver 우회 raw target 탐지 불가  | docs/specs/agent-identity.md#KG-21 |
| KG-22  | open     | low    | infra  | i18n 동적 호출 namespace EN 미정의 — KO 에만 있는 91 키가 EN 미정의, locale=en 시 부분 한글 잔재 | docs/specs/config.md#KG-22 |
| KG-23  | open     | low    | infra  | Cedar evaluate 호출이 Op ADT 로 wrapping 안 됨 — finite 선택 공간 강제 약화, LLM 직접 트리거 시 우회 위험 | docs/design/cedar-infra.md#KG-23 |
| KG-24  | open     | low    | infra  | CI-Y6 deny-path 자동화는 정책/엔진 측 deny 능력만 검증 — 호출처 정합성은 GV-Y1~Y4 (governance phase) 와 합쳐야 완전 | docs/design/cedar-infra.md#KG-24 |

## 통계

- FP 총 **67개** — open **0**, resolved **67**
- KG 총 **24개** — open **8**, resolved **16**
- Severity 분포 (open만): high 1 (KG-17) · medium 4 (KG-15, KG-16, KG-20, KG-21) · low 3 (KG-22, KG-23, KG-24)
