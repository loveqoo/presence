...
| FP-68  | resolved | medium   | tui  | admin 두 번째 세션 거부 시 원문 코드 노출 (`Access denied: admin-singleton`) | docs/ux/tui-slash-commands.md       |
...
| KG-26  | open     | medium | spec   | Admin quota 면제 spec/코드 갭 — I8 + agent-identity-model §9.3 은 admin 면제 + hard limit 명시, agent-governance.js submitUserAgent 는 admin 분기 없음 (Cedar P1 hybrid 잔류 의미론) | docs/design/governance-cedar.md#KG-26 |

## 통계

- FP 총 **71개** — open **0**, resolved **71**
- KG 총 **26개** — open **1**, resolved **25**
- Severity 분포 (open만): medium 1
...
- 2026-04-26: FP-70 후속 — `audit.js` 에 `getAuditStatus({ logPath, maxBytes, maxBackups })` 추가, `cedar/index.js` 에 `getSubsystemAuditStatus({ presenceDir })` 추가, `auth/cli.js` 에 `audit-status` 서브커맨드 (`npm run user -- audit-status`) 추가. 출력: 현재 size + maxBytes + 사용률(%) + 백업 개수 + 백업별 size + `.gz` 열람 안내 한 줄. 운영자 admin role 식별 + 서버 REST 추가 비용 회피, 운영자 도구 진입점 (`npm run user --`) 에 응집. CA14~CA17 회귀 (파일 부재 / rotation 후 size / logPath 검증 / Reader 동치). FP-70 의 issue 문서 중간 우선순위 제안 모두 해소.
- 2026-04-26: FP-68/69/70 resolved — `feature/cedar-governance-v2` 후속. FP-68 (medium/tui): `session-api.js` 의 admin singleton 거부 응답에 `code: 'AGENT_ACCESS_DENIED'` + `reason` 필드 추가, TUI `formatCreateError` (`sessions.js`) 가 `code === 'AGENT_ACCESS_DENIED' && reason === 'admin-singleton'` 분기로 `sessions_cmd.error.admin_singleton` i18n 메시지 표시. ko/en 양쪽 키 추가. SC4c 테스트로 한국어 메시지 검증. FP-69 (medium/tui): `local-tools.js` file_read 의 `error.file_not_found` 메시지에 `{{workspace}}` 변수 추가 — workingDir 동봉으로 LLM 이 첫 시도부터 워크스페이스 경계를 인지. ko/en 모두 갱신. local-tools.test.js 회귀 추가. FP-70 (low/infra): `audit.js` 의 `rotateIfNeeded` 가 logger 인자 받아 rotation 직후 한 줄 로그 (`[cedar-audit] rotation: <basename> → .1.gz (size: X.X MB, backups: N/M)`). `bootCedarSubsystem` / `createSubsystemAuditWriter` env 에 logger 옵션 추가. server/index.js 가 `console` 을 logger 로 전달. CA12 (logger 호출 횟수 + 메시지 포맷) + CA13 (logger 미주입 silent 호환) 회귀.
