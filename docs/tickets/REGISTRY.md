...
| FP-52  | open     | medium   | tui  | LLM truncation 시 유저에게 경고 없음 (KG-09 연계, 2026-04-16 reopened) | docs/ux/tui-chat-transcript.md      |
...
| FP-59  | open     | medium   | tui  | Plan EXEC 가 검증되지 않은 URL 을 tool_args 로 생성 (KG-12 연계) | docs/ux/tui-chat-transcript.md      |
| FP-60  | open     | medium   | tui  | Plan 마지막 ASK_LLM + RESPOND 누락 시 결과 폐기 (KG-13 연계)     | docs/ux/tui-chat-transcript.md      |
...
| KG-12  | open     | medium | core   | Plan EXEC tool_args 가 finite 선택 공간 밖 — planner 환각 침투 | docs/specs/planner.md#KG-12                    |
| KG-13  | open     | medium | core   | Plan 마지막 ASK_LLM + RESPOND 누락 허용 — 수렴 루프 vs LLM 실수 구분 불가 | docs/specs/planner.md#KG-13 |

## 통계

- FP 총 **60개** — open **3**, resolved **57**
- KG 총 **13개** — open **2**, resolved **11**
- Severity 분포 (open만): high **0**, medium **4**, low **0**
