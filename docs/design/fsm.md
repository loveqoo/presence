...
### D11. onUnknown 기본 'no-match' (개별) + product aggregation 규칙
...
- **product aggregation** 순서 (명시적 거부가 수락을 이긴다):
  - 1개 이상 `explicit` reject → 전체 Left, primaryReason = explicit 중 첫. 수락이 있어도 거부 우선
  - explicit 없고 `accepted` ≥ 1 → 전체 Right (state 병합, events concat, slot isolation)
  - 전원 `no-match` (또는 guard-failed 만) → 전체 Left, primaryReason = 'unhandled'
- Silent no-op 제거. "아무도 처리 못함" 이 명시적 신호.
- 근거: "명시적 거부는 항상 존중" 이 안전한 기본값. FSM 정의자가 "이 명령 무관심" 하려면 해당 transition 을 빼서 no-match 로 두면 된다. broadcast 가 필요하면 호출자가 Either.fold 로 직접 처리.
...
**step 오버라이드** (product 의 step, Phase 3 확정):
```
step(productFsm, namedState, command):
  ...
  allReasons = results.flatMap(([key, r]) =>
    r.isLeft ? r.value.reasons : []
  )
  firstExplicit = allReasons.find(r => r.kind === 'explicit')

  // D11: 명시적 거부가 수락을 이긴다
  if (firstExplicit):
    return Left({ command, state: namedState, reasons: allReasons,
  ...
  if (accepted.length >= 1):
    newState = productFsm.keys.reduce((acc, key) => {
      ...
      acc[key] = r.isRight ? r.value.state : namedState[key]   // slot isolation
      return acc
    }, {})
...
### Product rejection aggregation (D11 명세, Phase 3 확정)
```
∀ fsm ∈ children, r_fsm = step(fsm, s_fsm, c):

(a) ∃ fsm: r_fsm.reasons 에 kind='explicit' 존재
    → product step = Left(all_reasons, primaryReason = first_explicit.reason)
    (수락이 있어도 explicit 우선)

(b) ¬(a) ∧ ∃ fsm: r_fsm = Right(_)
    → product step = Right(merged_state, concat_events)

(c) ∀ fsm: r_fsm = Left(_) ∧ (a) 불성립
    → product step = Left(all_reasons, primaryReason = 'unhandled')
```

### Product state slot isolation
```
product step 이 accept (case b) 하면, reject 한 child FSM 의 state slot 은 불변.
case a (explicit 우선) 에서는 모든 slot 불변 (state 전체가 이전 값 유지).
```
