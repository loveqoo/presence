...
**Status**: 2026-04-24 v3. S1 구현 완료 — `feature/a2a-phase1` 브랜치 5 커밋. §8 에 구현 경로 테이블. 다음 단계 S2 (response + correlationId + 타임아웃 expire).
...
### 4.2 수신 session 라우팅 규칙 (v3)

agentId → session 매핑 (M1 단계):

- `SessionManager.findAgentSession(to)` 가 `type === SESSION_TYPE.AGENT` + `session.agentId === to` 인 entry 를 찾는다.
- **USER/AGENT dual-homed**: 같은 agentId (`{u}/default`) 가 USER session (대화) + AGENT session (delegate 대상) 양쪽에 존재할 수 있다. `session-api.js` 가 UserSession 을 `{u}/default` agentId 로 생성하고, `user-migration.js` 가 config.agents 에 default 를 seed 해 `scheduler-factory.js` 의 `registerAgentSessions` 가 AGENT session 도 생성하기 때문.
- `findAgentSession` 은 `type === AGENT` 만 선택 → SendTodo 는 delegate target 경로로만 라우팅. USER session (대화 흐름) 은 교란하지 않음.
- 0 session: `target-not-registered` 거부. N session (이론상 발생 없음 — agent 당 1): 방어적 `session-routing-ambiguous`.
