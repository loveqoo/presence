...
**Status**: 2026-04-24 v6. S1 + S2 + S3 완료. `feature/a2a-phase1` 브랜치. S3 는 사용자 지적 (2026-04-24) 으로 "다자 협업 via TODO" → 폐기, 대신 `list_agents` tool 만 추가. 다자 처리는 Parallel+Delegate 로 이미 해결됨을 명시. 다음 단계 S4 (큐 상한 + 재시작 회복).
...
**SendTodo 는 다른 용례**: 유저 대화 흐름에 비동기 TODO 를 투입하는 경로 (결과는 SYSTEM entry 로 대화창에 도착, 유저/LLM 이 관찰). "병렬 집계" 가 아닌 "비동기 맥락 투입" 의미론. 두 경로가 공존하되 목적이 다름.
...
- 동기 vs 비동기: TODO 는 본질적으로 비동기. 송신자는 `SendTodo` 호출 직후 결과 객체만 받고, 응답은 별도 이벤트 (S2 `todo_response`) 로 agent 에게 도달
...
```
Op.SendTodo(to, payload, { timeoutMs? })
...
### 4.5 `todo_response` event 처리 (v4 — S2)

송신 agent 의 EventActor drain 이 `todo_response` type 을 만나면:
...
`formatResponseMessage(event)` 형식 (v4):
...
turnLifecycle 은 EventActor env 에 필수 (SessionActors 가 주입). 미주입 + todo_response = 프로덕션 불변식 위반 → warn 로그 + skip (실제 발생 없음).
...
| **S3** ✅ 완료 (2026-04-24) | agent discovery | `list_agents` tool — 같은 유저 agent 목록 + description + capabilities 반환 (archived 제외). Delegate / SendTodo target 선택 편의. 다자 처리 자체는 Parallel+Delegate 로 이미 해결 (Q7 종결) | Q7 재평가 |
...
| SendTodo 인터프리터 | `packages/infra/src/interpreter/send-todo.js` |
| 수신 session 라우팅 (findAgentSession) | `packages/infra/src/infra/sessions/session-manager.js` |
| Op.SendTodo 정의 | `packages/core/src/core/op.js` |
| EVENT_TYPE / A2A 정책 상수 | `packages/core/src/core/policies.js` |
| 수신 측 상태 전이 (TODO_REQUEST 분기) | `packages/infra/src/infra/sessions/internal/session-actors.js` |
...
| todo_response drain + SYSTEM entry 추가 | `packages/infra/src/infra/actors/event-actor.js` `#handleTodoResponse` |
...
| TODO_RESPONSE event type / EXPIRE_TICK_MS 상수 | `packages/core/src/core/policies.js` |
| markCompleted/markExpired race 방어 + markOrphaned | `packages/infra/src/infra/a2a/a2a-queue-store.js` |
| SessionActors handleEventDone TODO_REQUEST 분기 (dispatchResponse 호출 + race 방어) | `packages/infra/src/infra/sessions/internal/session-actors.js` |
...
**활용**: planner agent persona 프롬프트에서 `list_agents` 를 호출 → 반환 목록으로 `Op.Delegate` target 또는 `Op.SendTodo` target 선택. 다자 처리는 `Op.Parallel([Delegate(a), Delegate(b), ...])` 로 한 번에 병렬 + 결과 집계.
