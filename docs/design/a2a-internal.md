...
A2aMessage = {
  ...
  error?: string,                // response.status !== 'completed' 일 때 사유
                                 // 가능 값: 'agent-error', 'queue-full' (S4 §6.5),
                                 //   'server-restart', 'server-restart-target-missing',
                                 //   'server-restart-enqueue-failed' (S4 §6.4),
                                 //   SEND_A2A_ERROR.* (정상 send 경로 거부)
  createdAt: ISO8601,
...
### 6.4 서버 재시작 (Q3) — v9 S4 구체화

- SQLite 영속화 — `~/.presence/users/{u}/memory/a2a-queue.db`
- **재시작 후 row 별 절차** (`UserContext.recoverA2aQueue({ sessionManager })`):
  - `processing` → `markFailed(id, 'server-restart')` + `dispatchResponse(status='failed', error='server-restart')` — 처리 중이던 session 이 유실되었으므로 멱등성 가정 불가. sender 부재 시 dispatchResponse 가 자동으로 `orphaned` 처리.
  - `pending` → `sessionManager.findAgentSession(toAgentId)` 분기:
    - `kind='ok'` → receiver event queue 에 a2a_request 재진입. enqueue 자체가 실패 (stale actor 등) 면 `markFailed(id, 'server-restart-enqueue-failed')` + sender response.
    - `kind='not-registered'`/`'ambiguous'` → `markFailed(id, 'server-restart-target-missing')` + sender response.
  - `completed` / `failed` / `expired` / `cancelled` / `orphaned` — 이력 보관 (cleanup 정책: 별도 KG, 잠정 14 일).
- **invariant** (recovery 정상 종료 후): silent 잔류 row 없음 — 모든 처리 대상 row 는 (a) `pending` + receiver event queue 진입 또는 (b) `failed` (server-restart*) 둘 중 하나.
- **bounded batch**: `listByStatus(status, { kind, limit: A2A.RECOVER_BATCH_MAX = 1000 })` 으로 startup 당 최대 1000 row 처리. 잔여는 다음 startup. `markFailed` 의 boolean 반환으로 idempotent — 두 번 처리되어도 무해.
- **feature flag**: `config.a2a.recoverOnStart` (default `true`). `false` 면 `recoverA2aQueue` 호출 자체 skip — 첫 배포 시 운영자가 검증 전 끄고 별도 도구로 DB 정리, 검증 후 활성화. 비가역 정리의 운영 rollback 경로.
- **두 부트 hook**: 메인 부트 (`packages/server/src/server/index.js`) + 인증 모드 lazy 부트 (`packages/server/src/server/user-context-manager.js getOrCreate`). 둘 다 `registerAgentSessions(...)` 직후 `await userContext.recoverA2aQueue({ sessionManager })` 호출. user-context-manager 는 single-flight 보호 (`#pending` Promise 캐싱) 추가 — 동시 첫 접근 시 UserContext.create + recovery 가 두 번 실행되지 않게.
- **race**: recovery 와 expire tick 동시 실행 — `markFailed` / `markExpired` 의 boolean 으로 보호. 먼저 전이 성공한 쪽만 dispatchResponse.

### 6.5 큐 상한 (Q10) — v9 S4 enforcement

- agent 당 `pending` 상한 100 (`A2A.QUEUE_MAX_PER_AGENT` — `policies.js`).
- **enforcement 위치**: `A2aQueueStore.enqueueRequest({ ..., maxPending = null })` 시그니처 확장. better-sqlite3 transaction 내부에서 `SELECT COUNT(*) WHERE to_agent_id=? AND status='pending' AND kind='request'` + INSERT 를 원자적으로 수행.
- 초과 시 row 는 `status='failed', error='queue-full'` 로 INSERT (audit 보존). caller (interpreter) 가 반환된 row 의 `status` 보고 분기.
- **race-free 불변식**: 트랜잭션 내 COUNT+INSERT 라 단일 프로세스 내 동시 sender 가 같은 receiver 로 enqueue 해도 100 초과 INSERT 불가능. 멀티 프로세스는 Phase 1 범위 밖.
- back-pressure 없음 — 초과는 즉시 fail.
...
| **S4** 진행 중 (2026-04-25) | 큐 상한 + 회복력 | `enqueueRequest({ maxPending })` 트랜잭션 enforcement + `recoverA2aQueue` 절차 (processing→failed, pending→re-enqueue/failed) + bounded batch (1000) + feature flag (`config.a2a.recoverOnStart`) + 두 부트 hook + UserContextManager single-flight 보호 | Q10 / Q3 |
...
### S4 구현 경로 (2026-04-25 진행 중)

| 역할 | 파일 |
|---|---|
| 트랜잭션 큐 상한 enforcement | `packages/infra/src/infra/a2a/a2a-queue-store.js` `enqueueRequest({ maxPending })` |
| listByStatus + bounded limit | `packages/infra/src/infra/a2a/a2a-queue-store.js` `listByStatus(status, { kind, limit })` |
| 정책 상수 (RECOVER_BATCH_MAX) | `packages/core/src/core/policies.js` `A2A.RECOVER_BATCH_MAX` |
| Recovery 절차 + feature flag | `packages/infra/src/infra/user-context.js` `recoverA2aQueue` |
| SEND_A2A_ERROR.QUEUE_FULL 추가 + interpreter row.status 분기 | `packages/infra/src/interpreter/send-a2a-message.js` |
| 메인 부트 hook | `packages/server/src/server/index.js` (registerAgentSessions 후) |
| 인증 모드 lazy 부트 hook + single-flight | `packages/server/src/server/user-context-manager.js` `getOrCreate` |
| humanize 매핑 (S4 신규 코드 4 개) | `packages/infra/src/infra/events.js` `formatResponseMessage` |
| i18n 키 추가 | `packages/infra/src/i18n/ko.json`, `en.json` (`a2a.error.*`) |
