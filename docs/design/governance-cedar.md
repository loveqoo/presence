...
- 실제 디렉토리 위치: `packages/infra/src/infra/authz/cedar/policies/` (KG-27 P4 에서 fix). `~/.presence/cedar/` 등 사용자 홈 경로는 X' (구현 안 함) — `cedar-infra.md §1.2`. 정책 변경 = 코드 PR 워크플로.
...
**상태**: resolved (v2.13). callable wrapper + 단순 single-flight + reloadStartedAt authoritative metadata + audit getPolicyVersion closure 패턴으로 hot reload 경로 완성. plan-reviewer (codex) 11 라운드 비판 리뷰 흡수 후 ship.

**해소 (P5)**:
1. **callable wrapper** (`evaluator-ref.js`): `createEvaluatorRef(initial, { version })` 가 closure-bound state (current/version/reloadedAt) 를 들고 있는 함수 wrapper. evaluator(args) 호출 시 매번 state.current 읽음 — 살아있는 세션 / 캐시된 UserContext / 인터프리터 closure 모두 자동 새 정책 사용. wrapper 는 `typeof === 'function'` 유지 — 5 주입 사이트 invariant 통과. `replace(newFn, version)` / `snapshot()` 메서드는 reload 전용.
2. **rebootCedarSubsystem** (`cedar/index.js`): 새 evaluator 함수 부팅만 — wrapper / auditWriter 미생성 (boot 시점 단일 인스턴스 재사용). 부팅 실패 시 throw → wrapper.replace 미호출 → 메모리 내 evaluator 미교체 (fail-safe rollback). 디스크 정책 파일 롤백은 운영자 책임 (코드 영향 없음).
3. **UserContextManager.reloadEvaluator** (`user-context-manager.js`): 단순 single-flight — `#reloadPending` Promise 진행 중이면 후속 호출자가 같은 결과 공유. `reloadStartedAt` 을 promise 생성 시점에 캡처 → leader/follower 가 같은 값. 응답 contract: 성공 `{ status: 'ok', version, reloadedAt, reloadStartedAt }`, 실패 `{ status: 'fail', error, activeVersion, activeReloadedAt }`.
4. **POST /api/admin/policy/reload + GET /policy/version** (`admin-router.js`): admin role 미들웨어 (`req.user.roles.includes('admin')` + ADMIN_USERNAME fallback). reload outcome 과 audit append 를 별도 try/catch — audit I/O 실패가 응답 오염 안 함 (round 9 H 흡수). audit JSONL 기록 (decision: success/fail).
5. **policyVersion 자동 첨부** (`audit.js`): `createAuditWriter({ getPolicyVersion })` closure 주입 — 모든 server-side audit entry 에 policyVersion 자동 첨부 (단일 진실 소스). 수동 기입 금지. CLI manual_approve 는 evaluator 무관 admin override → policyVersion=null 정합 (명시적 예외).
6. **CLI policy reload** (`cli-policy.js`): stub 대체. PRESENCE_ADMIN_TOKEN env 인증. 서버 미가동 / 권한 없음 / 부팅 실패 분기. ECONNREFUSED 특화 제거 — 모든 fetch 실패 동일 처리.
7. **invariants** (`agent-identity.md`): I-CEDAR-RELOAD-CALL-LINEARIZABLE (호출 단위 한 정책 버전), I-CEDAR-RELOAD-FAIL-SAFE (throw 시 미교체), I-CEDAR-RELOAD-EDGE-TRIGGER (reloadStartedAt follower 공유, 자동 follow-up 부재), I-CEDAR-AUDIT-VERSION (모든 entry policyVersion 첨부).

**의미론 명시**:
- 호출 단위 atomicity 만 보장 — 한 요청 흐름의 다중 evaluator 호출이 다른 정책 버전 가능 (fail-closed 의미론 보존).
- edge-trigger — 진행 중 reload 후 디스크 변경은 본 reload 미반영. 호출자가 변경 적용 검증하려면 명시적 두 번째 호출 후 reloadStartedAt 변화 관찰.
- replay 위험 (M3) — access token TTL 짧음 + audit 사후 추적으로 수용. nonce/short-lived admin token 후속 phase.
- ENV 의존 (M4) — process listing 노출 위험 명시. admin login file 후속 phase.

**테스트**: RL1~RL9 (cedar-reload.test.js — 정상/실패/single-flight/version monotonic/self-consistency/live closure/cached UC/edge-trigger metadata/audit policyVersion), AR1~AR7 (admin-router.test.js — REST 권한 + 정상 + audit fail isolation), CLI-X5~X6 (cedar-policy-cli.test.js), INV-CEDAR-RELOAD-WRAPPER / FAIL-SAFE / EDGE-TRIGGER / AUDIT-VERSION (정적 회귀). 4646 passed.

---

## Changelog

- **v2.15 (2026-05-03)**: §3.1 의 정책 디렉토리 placeholder ("v3 에서 fix") 정리 — 실제 경로 `packages/infra/src/infra/authz/cedar/policies/` 명시 + `~/.presence/cedar/` X' 분류 명시. KG-27 P4 + KG-28 P5 + FP-78 + FP-82 ship 후 stale 참조 해소. Cedar 거버넌스 종결 사이클 (`feature/kg-30-policy-lint-all`) 일부.
- **v2.14 (2026-04-30)**: FP-73 MVP resolved — admin token 자동 저장 (v2.13 M4 ENV 의존 해소). 신규 모듈 `admin-session.js` (`~/.presence/admin-session.json`, mode 0o600, 부모 디렉토리 0o700, atomicWriteJson, permission check `mode & 0o077`, decodeAccessExp, DRIFT_BUFFER 30s). 신규 `cli-admin.js` — `dispatchAdmin(login/logout/whoami)` + CliAdminError + mustChangePassword 차단 (파일 미저장 + passwd 안내 + exit 1). `cli-policy.js resolveAdminToken` 확장 — ENV → 파일 fallback → 자동 refresh + 401 retry (clock drift 대응. ENV 사용 중에는 retry 안 함). `cli-utils.js` 확장 — promptPassword/promptLine 이전 (cli.js refactor only). cli.js admin 2-단계 subcommand wiring + usage 갱신. 운영 가정 (release gate): single-admin / single-machine for admin / single-host server / passwd 가능 환경. 후속 phase 로 분리: file lock 동시 race 방어 (compare-and-swap + jti) / contract drift 감지 / credential rotation 운영 가이드 (단일/원격 토폴로지) / mustChangePassword 자동 변경 흐름 (login → change-password 단일 명령) / multi-instance 서버 토폴로지 revoke. plan-reviewer 11 라운드 (pre-MVP 6 + MVP 5) 비판 리뷰 거쳐 사용자 결정 (옵션 B) 으로 운영 규율 영역은 가이드 문서 흡수. 회귀: AS1~AS10 (단위) + CLI-X10~X17 (CLI 통합) + AR9~AR14b (server 통합) + INV-CEDAR-CLI-FILE-FALLBACK / INV-ADMIN-SESSION-MODE / INV-CEDAR-CLI-ADMIN-CMDS / INV-ADMIN-SESSION-DRIFT-BUFFER / INV-ADMIN-MUST-CHANGE-PASSWORD 정적 회귀. 4684 → 4775 passed (+91).
- **v2.13 (2026-04-29)**: KG-28 P5 resolved — Cedar policy hot reload. callable wrapper (`createEvaluatorRef`) 로 closure-bound state + 함수 인터페이스 유지 — 호출 사이트 변경 0, 살아있는 세션 / 캐시된 UC 자동 propagate. `bootCedarSubsystem` `{ evaluator, auditWriter }` destructuring contract — server boot 단일 auditWriter 인스턴스를 admin router 에 재사용. `rebootCedarSubsystem` 은 evaluator 함수만 부팅 — 부팅 실패 시 throw → wrapper.replace 미호출 = 메모리 미교체. UserContextManager.reloadEvaluator 단순 single-flight — `#reloadPending` follower 가 leader 의 reloadStartedAt 공유 (응답 single authoritative metadata). POST /api/admin/policy/reload + GET /policy/version 신규 (admin role 미들웨어). audit `getPolicyVersion` closure 자동 첨부 — 모든 server-side entry policyVersion 첨부 (단일 진실 소스). audit append 는 별도 try/catch — I/O 실패가 reload outcome 오염 안 함 (round 9 H 흡수). plan-reviewer (codex) 11 라운드 비판 리뷰 흡수 후 ship — H1 (wrapper), H2 (타입 계약), H3 (호출 단위 선형화), H4 (single-flight follower), L1 (호출 사이트 변경 0), M1 (rollback honest), M2 (role 기반), M3 (replay 수용), M4 (ENV 명시), M5 (audit version), M6 (GET version), M7 (테스트 보강) + round 5~11 결함 (applied 필드 제거, reloadStartedAt authoritative, audit single source, ECONNREFUSED 일반화 등). 테스트: RL1~RL9 + AR1~AR7 + CLI-X5~X6 + INV-CEDAR-RELOAD-* + INV-CEDAR-AUDIT-VERSION. 4646 passed.
