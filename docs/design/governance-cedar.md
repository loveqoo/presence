...
**Status**: 2026-04-25 v2. §1.0 메타 결정 신설 (의미론 집중도 = 옵션 Y "최소"). §1.1~1.3 sub-결정은 Y 에서 자동 도출 (1.1-A / 1.2-B / 1.3-B). §2 seed 를 minimal 로 단순화. §3~§4 구현 범위 축소. v1 (2026-04-23) 의 무한 리뷰 원인 — 메타 결정이 implicit 한 채 sub-결정 3 개 동시 토론 — 을 메타 결정 명문화로 차단.
...
- 단점: 의미론이 두 층 (Cedar / 코드) 에 분산. 첫 인상이 "Cedar 가 trivial 해서 도입 가치 의문"
...
- `packages/infra/src/authz/defaults/00-base.cedar` — minimal RBAC seed 작성 (§2.2)
- `packages/infra/src/authz/defaults/schema.cedarschema` — `create_agent` action 추가 (context 확장 없음)
- `packages/infra/src/infra/agents/agent-governance.js` — `submitUserAgent` 진입에 Cedar evaluate 호출 (RBAC 게이트). 기존 quota / autoApprove / hard limit 분기 **유지** (코드에서 처리)
- `packages/infra/src/infra/auth/cli.js` — `agent add` 에 Cedar boot + evaluate + audit. `agent approve` 에 manual_approve audit
- 기존 governance 테스트는 대부분 그대로 (의미론 미변동). Cedar 호출 추가에 대한 단위 테스트만 추가

### 3.2 건드리지 않는 파일

- `packages/infra/src/authz/entity-bag.js` — context 조립 확장 거의 없음 (옵션 Y)
- `canAccessAgent` 시그니처 — 후속 플랜 (A2A authorization, KG-17) 에서
- `admin-bootstrap.js` — 서버 부팅 경로 무변화
- 5 진입점 — runtime 무변화

### 3.3 옵션 Y 의 trivial 함이 곧 가치인 이유

옵션 Y 는 Cedar 의 **위치** 만 박는다. 의미론은 코드에 그대로. 이 단계의 목표는 "Cedar 가 모든 governance 결정을 흡수" 가 아니라 **"Cedar evaluate 호출이 어디에 박히는지 확정 + audit 경로 작동 검증"**. 가치 검증은 다음 phase 부터:
- C5 (A2A authorization, KG-17): Cedar 가 A2A JWT 호출의 RBAC 게이트로 첫 활용
- C6 (capability migration): capability proof 검증의 자연스러운 도구

옵션 Y → X 마이그레이션 비용은 **한 곳 (`agent-governance.js`) + 정책 파일** 에 한정. 사용처가 늘기 전에 옮길 수 있음.
