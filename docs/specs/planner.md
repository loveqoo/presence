...
- E6. **`$N` 참조의 부분 구현**: `PLAN_RULES`의 이전 Rule 6에 있던 `$N` 참조 가이드는
  삭제되고 "ASK_LLM 마지막이면 RESPOND 필수" 가이드로 교체되었다 (2026-04-16).
  실제 `$N` 치환은 EXEC의 `tool_args` 필드에 한해 구현되어 있다
  (`op-handler.js` `ExecOp.resolveToolArgs` — `str.replace(/\$(\d+)/g, ...)` 로
  이전 스텝 결과를 참조). ASK_LLM의 `prompt` 필드는 미구현이며,
  리터럴 `$1`/`$2` 문자가 그대로 내부 LLM에 전달된다.
  PLAN_RULES 가이드 삭제는 "LLM이 자율적으로 $N을 사용하지 않도록" 하기 위한 것이며,
  EXEC.tool_args의 $N은 내부적으로만 사용해야 한다 — 프롬프트에는 노출하지 않는다.
...
- 2026-04-25: E6 갱신 — EXEC.tool_args $N 치환 실제 구현 사실 반영, ASK_LLM.prompt 만 미구현으로 정정 (기존 "전혀 구현되지 않았다"는 부정확)
