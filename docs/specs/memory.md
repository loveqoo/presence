...
mem0(`mem0ai/oss`) 기반으로 완전 전환 완료. 서버 레벨 단일 인스턴스, 모든 메서드에 `agentId` 전달로 격리.
...
- I2. **agentId 격리**: `Memory.search()`, `Memory.add()`, `Memory.allNodes()`, `Memory.clearAll()`, `Memory.removeOlderThan()` — 모든 메서드는 반드시 `agentId`를 파라미터로 받는다. `agentId`는 canonical form `{username}/{agentName}`이며 mem0 내부에서 `opts.userId`로 전달된다. 같은 유저의 서로 다른 agent가 qualified key 충돌 없이 독립 격리된다. 유저 격리는 qualifier 구조로 자동 달성된다.
- I2a. **search() 반환 형식과 변환**: `Memory.search(agentId, input, limit)` / `MemoryActor.recall()`의 반환 형식은 `{ label: string }[]`이다. Executor는 이를 `memories.map(n => n.label)`로 변환하여 `state.context.memories`에 `string[]`로 저장한다 (`executor.js:44`). 실패 시 `[]`로 저장 (`executor.js:49`). 소비처(`planner.js`, `useAgentState.js`)는 `string[]`를 가정한다.
...
- I4. **MemoryActor가 비동기 경계**: 메모리 **상태 변경**(recall 결과 반영, save)은 MemoryActor를 통해서만 수행된다. Agent/Interpreter가 `Memory` 인스턴스를 직접 호출하지 않는다. **단, 읽기 전용 조회(`/memory list` 슬래시 커맨드 등)는 `Memory` 인스턴스(`memory.allNodes(agentId)`)를 직접 호출할 수 있다.** I9 참조.
...
- I9. **슬래시 커맨드 접근**: `/memory list`에는 두 경로가 존재한다.
  - (a) **서버 측** (`session-api.js`): `session.agentId` 기준으로 `memory.allNodes(agentId)` 직접 호출 (MemoryActor 미경유). I4의 읽기 전용 예외에 해당. 다른 agent의 메모리에 접근할 수 없다.
  - (b) **Repl 측** (`core/repl.js` `cmdMemory()`): `this.memory.allNodes(this.agentId)`로 agentId를 전달한다. I2 불변식 준수. TUI `slash-commands/memory.js`도 `ctx.agentId`를 받아 모든 memory 호출에 전달한다.
...
- E4. agentId가 다른 두 agent가 같은 Memory 인스턴스에 recall 요청 → mem0 내부에서 agentId(`{username}/{agentName}`) 기반으로 격리. 같은 유저라도 agentName이 다르면 교차 노출 없음.
- E4a. **orphan agentId**: 과거에 존재했다가 제거/rename된 agent의 mem0 엔트리는 삭제되지 않는다. 같은 agent name을 재생성하면 동일 qualified key로 재조회 가능하다 ("영구 미조회"가 아니라 "같은 이름 재사용 전까지 미조회"). 완전 청소는 운영자가 `config.memory.path` 를 수동으로 비우는 방법으로만 가능.
...
- 2026-04-24: data-scope-alignment 완료 반영 — I2 격리 단위를 userId → agentId로 전환. qualified key `{username}/{agentName}` 구조로 유저 간 격리가 자동 달성됨을 명시. I2a/I4/I9 파라미터명 갱신. E4를 agentId 기준으로 재작성, E4a(orphan agentId 정책) 신규 추가.
