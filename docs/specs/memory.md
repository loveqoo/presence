...
- I9. **슬래시 커맨드 접근**: `/memory list`에는 두 경로가 존재한다.
  ...
  - (b) **Repl 측** (`core/repl.js` `cmdMemory()`): `this.memory.allNodes()`를 userId 인자 없이 호출 (`repl.js:76`). I2 불변식(모든 메서드는 userId 필수)의 Known Gap. 단, Repl은 로컬 모드 전용이며 현 운영 TUI에서는 `memory: null`로 미사용 경로다.
