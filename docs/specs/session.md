...
- I16. **A2A 수신 session 라우팅**: `SessionManager.findAgentSession(agentId)` 는 `type === SESSION_TYPE.AGENT` + `session.agentId === agentId` 인 entry 만 선택하고 tagged union 을 반환한다. 반환 형태: `{ kind: 'ok', entry }` (정확히 1개 매치), `{ kind: 'not-registered', entry: null }` (0개 매치), `{ kind: 'ambiguous', entry: null }` (2개 이상 — 이론상 발생 없음, 방어). 같은 agentId 로 USER session (대화) 과 AGENT session (delegate 대상) 이 공존하는 dual-homed 상황 (`{u}/default`, manager 등) 에서도 AGENT session 만 선택 — `SendTodo` 는 delegate target 경로로만 라우팅되고 UserSession 의 대화 흐름을 교란하지 않는다.
...
- I16 → (미커버) ⚠️ findAgentSession dual-homed 라우팅 단위 테스트 없음. `packages/infra/test/session-manager.test.js` 에 USER/AGENT dual-homed 시나리오 + not-registered/ambiguous 경로 테스트 필요
...
- `packages/infra/src/interpreter/send-todo.js` — SendTodo 인터프리터 (validateTarget + enqueue + eventActor.enqueue)
