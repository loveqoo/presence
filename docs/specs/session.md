...
- I1. **생성 순서 불변**: `Session` 생성자의 `init*` 단계는 항상 `workingDir 결정(생성자 직접) → initState → initTurnControl → initPersistence → restoreState(workingDir 복원 포함) → initToolRegistry → initInterpreter → initActors → initAgent → initScheduler → initTools → initMonitor` 순서로 실행된다. `workingDir`은 생성자에서 `resolveWorkingDir(userContext, opts)`를 통해 확정되며, `restoreState` 시 persistence에 저장된 값이 있으면 해당 값이 최우선으로 덮어쓴다.
...
- I14. **세션 workingDir은 생성 시 확정**: 세션의 `workingDir`은 `POST /sessions` body → `config.tools.allowedDirs[0]` fallback 순으로 결정된다. fallback 시 `pendingBackfill=true`이며, 첫 WS join 시 TUI `cwd`로 1회 덮어쓴다. `process.cwd()`는 세션 `workingDir` 결정에 사용되지 않는다. `allowedDirs`가 비어 있으면 세션 생성 자체가 실패한다. 세부 계약은 `tui-server-contract.md`의 INV-WD-CHAIN/BOUND/BACKFILL을 참조.
- I15. **UserSession은 workingDir을 persistence에 저장**: `UserSession.flushPersistence()`는 `workingDir`과 `pendingBackfill` 필드를 state.json에 함께 저장한다. 복원(restoreState) 시 저장된 `workingDir`이 있으면 생성자에서 계산된 값을 덮어쓴다. `pendingBackfill`도 함께 복원된다.
...
- I10. **세션 destroy는 메모리를 건드리지 않음**: `session.cleanup()`은 **세션 범위 영속화**(state.json 등)만 제거한다. `Memory`는 `userId` 단위로 격리되어 있고 한 유저가 여러 세션을 가질 수 있으므로, 한 세션 삭제가 유저의 `Memory`를 지우면 **다른 세션의 맥락이 함께 파괴**된다. 메모리 삭제는 (a) 유저가 `/memory clear` 슬래시 커맨드로 명시 삭제하거나, (b) 관리자 CLI의 `removeUserCompletely`로 유저 자체를 삭제할 때만 일어난다.
- I13. **관리자 CLI 유저 삭제는 3단계 원자 절차**: `removeUserCompletely({ store, memory, username, userDir })`는 아래 순서로 실행된다.
  1. `memory.clearAll(username)` — mem0에서 userId 기준 데이터 전량 삭제. `memory === null`이면 skip (embed credentials 없는 환경).
  2. `userDir` 존재 시 `rmSync(recursive, force)` — 사용자 데이터 디렉토리 재귀 삭제.
  3. `store.removeUser(username)` — users.json에서 레코드 제거.
  단계 1의 `memory.clearAll` throw는 best effort로 처리되어 단계 2, 3의 진행을 막지 않는다. 존재하지 않는 유저에 대한 호출은 `User not found` 에러를 throw한다.
...
- E14. `POST /sessions`에 `workingDir`이 `allowedDirs` 밖이면 → 세션 생성 실패, HTTP 400.
- E15. `config.tools.allowedDirs`가 빈 배열이면 → `POST /sessions` 시 세션 생성 실패 (`workingDir not resolvable`), HTTP 400.
- E16. WS join 메시지의 `cwd`가 `allowedDirs` 밖이면 → `WS_CLOSE.WORKING_DIR_INVALID(4004)` close. backfill 없음, 재연결 없음.
...
- I14, I15 → `packages/infra/test/session.test.js` SD6–SD10, `packages/infra/test/persistence.test.js` 5/6, `packages/server/test/server.test.js` S20b–S20d
- E14 → `packages/server/test/server.test.js` S20c, `packages/infra/test/session.test.js` SD8
- E15 → `packages/infra/test/session.test.js` SD9
- E16 → `packages/server/test/server.test.js` (WS 4004 경로) ⚠️ 직접 close 코드 assertion 확인 필요
