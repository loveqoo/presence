...
**추가 해소 (2026-04-16, FP-52 재개 → 재해소)**

KG-09 해소 시 `truncated` 플래그가 `SseParser`/`LLMClient` 까지 전파되었으나, TUI 까지의 경로가 미완이었다 (FP-52, 2026-04-16 재개).

- `safeJsonParse` (validate.js) 에 truncation 휴리스틱 추가: JSON parse 실패 시 응답이 200자 이상이고 `}`, `]`, `"` 로 끝나지 않으면 `TurnError(msg, kind, truncated=true)` 반환.
- `TurnError` 에 `truncated` 파라미터 추가 (기본값 false). 기존 호출처에 영향 없음.
- `buildRetryPrompt` 가 error 객체를 직접 받아 `truncated` 일 때 "response too long, use shorter" 힌트를 retry 프롬프트에 포함.
- `Planner.retryOrFail` 이 `_retry` 상태에 `truncated` 플래그 추가.
- `useAgentState` 가 `truncated` 분기 → `status.retry_truncated` ("재시도 N/M — 응답 절단") 표시.

E1 에러 메시지 개선: "Unterminated string in JSON at position 657" → "... (response likely truncated at 658 chars — try a shorter response)".

## 테스트 커버리지

- I1 → 구현 완료. 전용 단위 테스트 없음 ⚠️
- I2 → `SseParser` 구현 완료 (`finish_reason: "length"` → `truncated: true`). `agent.test.js` T7 블록에 truncation 탐지 4 assertion 추가
...
- 2026-04-16: FP-52 재해소 — safeJsonParse truncation 휴리스틱 + TurnError.truncated 파라미터 + buildRetryPrompt truncation 힌트 + useAgentState retry cause 분기
