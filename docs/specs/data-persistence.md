...
└── users/
    └── {username}/
        ...
        ├── a2a-relationships.db        ← A2aRelationshipStore (SQLite, 영속 관계 컨테이너 메타, KG-37 resolved)
        ├── agents/
        │   └── {agentName}/
        │       └── sessions/
        │           └── {sessionId}/
...
- I8. **EphemeralSession은 no-op persistence (만남 실행 컨텍스트)**: `scheduled`, `agent` 세션의 `flushPersistence()`, `clearPersistence()`는 no-op. 디스크 I/O 없음. SESSION_TYPE.agent 자체는 여전히 휘발성 (만남 단위 컨텍스트). 영속 관계 컨테이너 메타는 `a2a_relationships` 테이블 (`a2a-relationship-store.js`) 가 분담 — 모델 B 의 두 결 (만남 = 휘발 / 관계 = 영속) 을 두 저장 계층으로 명확히 분리. KG-37 resolved (Phase 3).
...
- I12 → `packages/infra/test/session.test.js` SD6 (workingDir 미저장, pendingBackfill 없음 assertion)
...
- I13 → (미커버) ⚠️ A2aQueueStore 단위 테스트 없음. `packages/infra/test/a2a-queue-store.test.js` 필요 (enqueueRequest / markProcessing 멱등 / markCompleted / markFailed / enqueueResponse final-only / listExpired / markExpired race / markOrphaned / close 포함)
...
- `packages/infra/src/infra/a2a/a2a-relationship-store.js` — A2aRelationshipStore (SQLite, 영속 관계 컨테이너 메타, KG-37 resolved). composite PK (local_agent_id, peer_agent_id) 1:1 고정. upsertOnFirstMeeting / recordMeeting / closeRelationship / refreshCards.
...
- 2026-04-23: I12 재작성 — W1(cb6c59a) 반영. workingDir/pendingBackfill 영속화 → 미영속화. 복원 시 재계산 규칙. 테스트 커버리지 갱신.
- 2026-04-10: 초기 작성
...
- 2026-05-05: KG-37 resolved — A2aRelationshipStore 도입 + UserContext 부팅 + a2a-router 카드 교환 게이트 통합. 모델 B 의 만남/관계 두 lifecycle 이 분리된 저장 계층으로 표현됨. 파일 경로 트리에 a2a-relationships.db 추가. 관련 코드에 a2a-relationship-store.js 추가.
