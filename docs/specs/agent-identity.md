...
- I-CEDAR-QUOTA. **Quota 의미론은 Cedar 정책에서 평가** (governance-cedar v2.4 §X → v2.11 §X5): `submitUserAgent` 가 `Op.CheckAccess` 를 `context: { currentCount, maxAgents, isAdmin, hardLimit }` 와 함께 호출. 두 forbid 정책: `10-quota.cedar` (`!isAdmin && currentCount >= maxAgents`), `11-admin-limit.cedar` (`isAdmin && currentCount >= hardLimit`). `isAdmin = (requester === ADMIN_USERNAME)`. `hardLimit` 은 `PRESENCE_ADMIN_AGENT_HARD_LIMIT` 환경변수 (부재 시 50). 호출 순서: validate → duplicate (Cedar 전 단락) → count/policies 계산 → Cedar → `classifyDeny(matchedPolicies)` + `interpretCedarDecision` 매핑. Cedar deny 의 governance 매핑은 `classifyDeny` priority 분류로 결정 (I-CEDAR-DENY-CLASSIFICATION 참조): operator (50-*) / protect (30-/31-) / admin-limit (11-) deny → terminal `STATUS.DENIED`; quota (10-) deny → `STATUS.PENDING` (admin 이 quota 상향 검토 가능 — 유일 PENDING); 빈 매치 deny → `DENIED(unspecified)` fail-closed. `decision='deny' && errors.length > 0` → `DENIED(evaluator-error)`. `decision='allow' && !autoApprove` → `PENDING(manual-review)`; `decision='allow' && autoApprove` → `APPROVED`. 운영자 정책 슬롯 (`50-*.cedar`) 은 KG-27 (P4) 해소로 정상 부팅 — cedar-wasm staticPolicies 맵 입력으로 `matchedPolicies` 파일 식별 보장. boot 차단 가드 제거됨. admin CLI 추가: `npm run user -- policy lint --file <path>` (parse + schema validate), `policy list` (카테고리 표).
...
- I-CEDAR-MATCH-ID. **`matchedPolicies` 는 정책 파일 basename 의 부분집합** (governance-cedar v2.11 §X5, codex M8 약화): cedar-wasm 에 `staticPolicies` 맵 형태로 입력할 때만 보장됨. basename 예: `'10-quota'`, `'50-foo'`, `'00-base-2'` (단일 파일에 다중 statement 시). ordering 및 per-statement 단위 진단은 unspecified — `classifyDeny` 는 priority 기반 다중 매치를 first-match 가정 없이 결정.

- I-CEDAR-DENY-CLASSIFICATION. **Cedar deny 의 governance 매핑은 매치 ID prefix 로 결정** (governance-cedar v2.11 §X5, codex H3): `classifyDeny(matchedPolicies)` 가 priority 분류를 수행. operator (`50-*`) / protect (`30-` / `31-`) / admin-limit (`11-`) deny 는 `STATUS.DENIED` terminal — admin 검토 큐에 진입하지 않음. quota (`10-`) deny 만 `STATUS.PENDING` (admin 이 quota 상향 가능). 매치 없음 deny → `DENIED(unspecified)` fail-closed (silent fail-open 차단).

- I-CEDAR-RELOAD-CALL-LINEARIZABLE. **evaluator 호출 한 번은 한 정책 버전 사용 (호출 단위 선형화)** (governance-cedar v2.13 §X6): callable wrapper (`evaluator-ref.js`) 가 호출 시점에 `state.current` 를 읽어 해당 호출을 완료. 진행 중 `cedar.isAuthorized` 호출은 캡처 시점 evaluator 로 완료됨. 한 요청 흐름에서 다중 evaluator 호출이 서로 다른 정책 버전을 볼 수 있음 — 단, 단일 호출 내 버전 변경은 없음. fail-closed 의미론 보존 (버전 전환이 중간에 끊기지 않음). **구체적 다중 호출 경로**: `POST /sessions/:id/chat` 핸들러가 (1) `attachSessionMiddleware` 에서 `canAccessAgent` (access_agent) 를 호출하고, (2) slash command `/persona set|reset` 시 `handleSlashCommand` 내부에서 `set_persona` Cedar 게이트를 추가 호출한다. 두 호출이 reload 경계에 걸리면 서로 다른 정책 버전을 볼 수 있음 — 이는 이 불변식의 의도된 허용 범위.

- I-CEDAR-RELOAD-FAIL-SAFE. **rebootCedarSubsystem throw 시 메모리 내 evaluator 미교체** (governance-cedar v2.13 §X6): `rebootCedarSubsystem` 이 throw 하면 호출자 (`UserContextManager.#doReload`) 가 `wrapper.replace` 를 호출하지 않음 → 메모리 내 evaluator 는 이전 버전 그대로 유지 (fail-safe rollback). 디스크 정책 파일 상태는 이 불변식의 범위 밖 — 운영자가 명시적으로 관리해야 한다.

- I-CEDAR-RELOAD-EDGE-TRIGGER. **reload 는 호출 시점 기준 부팅 — 후속 트리거는 같은 promise 결과 공유** (governance-cedar v2.13 §X6): `UserContextManager.reloadEvaluator` 는 진행 중 reload(`#reloadPending`)가 있으면 후속 호출자가 같은 promise 결과를 공유. `reloadStartedAt` 은 leader promise 생성 시점에 캡처되며 follower 에게 동일 값이 반환됨 — 호출자가 새 reload 시작 여부를 판정하려면 이전 응답의 `reloadStartedAt` 과 비교. 호출 후 디스크 변경은 본 reload 에 미반영. 응답 `version` 은 항상 `wrapper.current` 와 일치 (authoritative). 후속 변경 적용은 명시적 재호출 필요 (자동 follow-up 부재).

- I-CEDAR-AUDIT-VERSION. **모든 server 프로세스의 authz audit entry 에 policyVersion 필드 첨부** (governance-cedar v2.13 §X6): `createAuditWriter({ getPolicyVersion })` closure 가 `append` 시점마다 `policyVersion` 을 자동 주입 — reload 전후 결정을 audit 로그에서 식별 가능. 단일 진실 소스 구현 경로: `bootCedarSubsystem` 이 단일 `auditWriter` 인스턴스를 생성하고 server boot 시 `PresenceServer.#auditWriter` 에 저장 → `UserContextManager` / `adminRouter` 양쪽에 동일 인스턴스 주입. 5 진입점의 evaluator 호출은 `bootCedarSubsystem` 이 생성한 `innerEvaluator` 의 내부 closure 를 경유하므로 같은 `auditWriter` 에 기록됨 — 별도 auditWriter 인스턴스 생성 불가. 수동 기입 금지. 명시적 예외: CLI 단발 프로세스 (`cmdAgentApprove manual_approve`) 는 `createSubsystemAuditWriter` 로 별도 writer 생성하지만 `getPolicyVersion` 미주입 → `policyVersion=null` 이 정합 (admin override, 정책 무관).
...
- **KG-28** (REGISTRY: KG-28): ~~Cedar policy hot reload 미구현. 정책 변경 시 서버 재시작 필요 (`cedar-infra.md §1.7 Y'` 초기 결정). 운영자가 정책을 추가/수정한 후 적용하려면 서버 전체 재시작 불가피 — 운영 중단 없는 정책 갱신 필요.~~ resolved by feature/cedar-governance-v2 (2026-04-29). callable wrapper (`createEvaluatorRef`) + single-flight reload (`UserContextManager.reloadEvaluator`) + `POST /api/admin/policy/reload` REST API 로 서버 재시작 없이 정책 교체 가능. fail-safe rollback (rebootCedarSubsystem throw 시 wrapper 미교체), edge-trigger (진행 중 reload 의 follower 가 leader 결과 공유), audit policyVersion 자동 첨부. I-CEDAR-RELOAD-CALL-LINEARIZABLE / I-CEDAR-RELOAD-FAIL-SAFE / I-CEDAR-RELOAD-EDGE-TRIGGER / I-CEDAR-AUDIT-VERSION 불변식으로 승격.
...
- I-CEDAR-QUOTA → `packages/infra/test/agent-governance.test.js` GV-X1~GV-X19, `packages/infra/test/cedar-evaluator.test.js` CE7~CE11, `packages/infra/test/cedar-boot.test.js` CB7/CB-X1/X2/X3, `packages/infra/test/check-access-interpreter.test.js` CK4, `test/regression/cedar-quota-policy.test.js` (INV-CEDAR-QUOTA-POLICY + INV-CEDAR-ADMIN-EXEMPT + INV-SUBMIT-USER-AGENT-CONTEXT + INV-CREATE-AGENT-CALLERS + INV-CEDAR-POLICY-MAP + INV-INTERPRET-MATCHED-POLICIES + INV-DENIED-VS-PENDING)
...
- I-CEDAR-MATCH-ID → `test/regression/cedar-quota-policy.test.js` INV-CEDAR-POLICY-MAP (staticPolicies 맵 입력으로 basename 반환 검증) + INV-INTERPRET-MATCHED-POLICIES (classifyDeny 다중 매치 결정 검증). `packages/infra/test/agent-governance.test.js` GV-X16~GV-X19 (50-* operator deny 시나리오)
- I-CEDAR-DENY-CLASSIFICATION → `test/regression/cedar-quota-policy.test.js` INV-DENIED-VS-PENDING (operator/protect/admin-limit → DENIED terminal, quota → PENDING 분기 정적 회귀). `packages/infra/test/agent-governance.test.js` GV-X1~GV-X19 (분류별 STATUS 매핑 통합). `packages/infra/test/cedar-boot.test.js` CB-X1/X2/X3 (50-* 슬롯 부팅 후 deny 분류 동작)
- I-CEDAR-RELOAD-CALL-LINEARIZABLE → `packages/infra/test/cedar-reload.test.js` RL3 (single-flight follower 가 leader 결과 공유), RL6 (live closure — 호출 시점 state.current 읽음 확인). `test/regression/` INV-CEDAR-RELOAD-WRAPPER (evaluator wrapper 타입 계약 정적 회귀)
- I-CEDAR-RELOAD-FAIL-SAFE → `packages/infra/test/cedar-reload.test.js` RL2 (rebootCedarSubsystem throw 시 wrapper 버전 미교체 + 이전 evaluator 동작 유지). `test/regression/` INV-CEDAR-RELOAD-FAIL-SAFE (rebootCedarSubsystem 실패 경로 wrapper.replace 미호출 정적 회귀)
- I-CEDAR-RELOAD-EDGE-TRIGGER → `packages/infra/test/cedar-reload.test.js` RL3 (follower reloadStartedAt = leader 값), RL8 (edge-trigger metadata: leader/follower 동일 reloadStartedAt). `test/regression/` INV-CEDAR-RELOAD-EDGE-TRIGGER (reloadPending follower 공유 경로 정적 회귀)
- I-CEDAR-AUDIT-VERSION → `packages/infra/test/cedar-reload.test.js` RL9 (audit append 시 policyVersion 자동 첨부). `packages/server/test/admin-router.test.js` AR5 (reload audit entry 에 policyVersion 포함). `test/regression/` INV-CEDAR-AUDIT-VERSION (getPolicyVersion closure 주입 경로 정적 회귀). CLI manual_approve policyVersion=null 은 별도 테스트 없음 — createSubsystemAuditWriter 의 getPolicyVersion 미주입으로 간접 보장
...
- `packages/infra/src/infra/authz/cedar/index.js` — `rebootCedarSubsystem`: evaluator 함수만 부팅, throw 시 wrapper.replace 미호출 (I-CEDAR-RELOAD-FAIL-SAFE)
- `packages/server/src/server/user-context-manager.js` — `reloadEvaluator` / `#doReload`: single-flight `#reloadPending`, reloadStartedAt leader 캡처 (I-CEDAR-RELOAD-EDGE-TRIGGER)
- `packages/server/src/server/admin-router.js` — POST /api/admin/policy/reload + GET /policy/version, reload outcome / audit 분리 try/catch
- `packages/infra/src/infra/authz/cedar/audit.js` — `createAuditWriter({ getPolicyVersion })`: append 시 policyVersion 자동 첨부 closure (I-CEDAR-AUDIT-VERSION)
