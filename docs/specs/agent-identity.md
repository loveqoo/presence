...
- I-CEDAR-RELOAD-CALL-LINEARIZABLE. **evaluator 호출 한 번은 한 정책 버전 사용 (호출 단위 선형화)** (governance-cedar v2.13 §X6): callable wrapper (`evaluator-ref.js`) 가 호출 시점에 `state.current` 를 읽어 해당 호출을 완료. 진행 중 `cedar.isAuthorized` 호출은 캡처 시점 evaluator 로 완료됨. 한 요청 흐름에서 다중 evaluator 호출이 서로 다른 정책 버전을 볼 수 있음 — 단, 단일 호출 내 버전 변경은 없음. fail-closed 의미론 보존 (버전 전환이 중간에 끊기지 않음).
...
- I-CEDAR-AUDIT-VERSION. **모든 server 프로세스의 authz audit entry 에 policyVersion 필드 첨부** (governance-cedar v2.13 §X6): `createAuditWriter({ getPolicyVersion })` closure 가 `append` 시점마다 `policyVersion` 을 자동 주입 — reload 전후 결정을 audit 로그에서 식별 가능. 단일 진실 소스: server boot 의 단일 `auditWriter` 인스턴스 (수동 기입 금지). 명시적 예외: CLI 단발 프로세스 (`cmdAgentApprove manual_approve`) 는 wrapper 를 생성하지 않으므로 `getPolicyVersion` 부재 → `policyVersion=null` 이 정합 (admin override, 정책 무관).
