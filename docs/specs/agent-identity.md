...
- I-CEDAR-PERSONA. **`/persona set|reset` 은 `set_persona` Cedar action 을 경유** (governance-cedar v2.8 §X3): `slash-commands.js` 의 `persona` 핸들러가 `set` / `reset` 서브커맨드 실행 전 `Op.CheckAccess({ action: 'set_persona', principal: { type: 'LocalUser', id: jwtSub }, resource: { type: 'Agent', id: primaryAgentId }, context: { isAdmin, reservedOwner } })` 를 호출하고 `runCheckAccess(evaluator, op)` 로 Cedar 평가를 실행한다. 현재 의미 제약 forbid 정책 없음 — ownership 은 session middleware 의 `canAccessAgent` 가 이미 보장. `set_persona` 의 핵심 목적은 audit JSONL 기록(governance trace). Cedar deny 시 "Persona change denied" 응답. `evaluator` / `jwtSub` / `agentId` 중 하나라도 없으면 게이트 skip (CLI · 테스트 호환 — fail-open). `/persona show` 는 read-only 이므로 Cedar 호출 없음.
...
- I-CEDAR-PERSONA → `packages/infra/test/cedar-evaluator.test.js` CE14 (set_persona permit + audit 1건 기록), `packages/infra/test/cedar-boot.test.js` CB12 (실 자산 부팅 후 set_persona allow + audit 기록), `packages/server/test/server.test.js` S7b (mock evaluator 경로 /persona show·set·reset 정상 동작 — evaluator 미전달로 게이트 skip 경로 포함). `/persona show` 의 Cedar 비호출은 별도 테스트 없음 — S7b 로 간접 검증.
