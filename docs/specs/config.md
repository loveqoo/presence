...
| `a2a.recoverOnStart` | `true` | 서버 시작 시 A2A 큐 재시작 회복 활성화 여부. `false`이면 `recoverA2aQueue` skip — 첫 배포/운영 rollback 경로. `UserContextManager.getOrCreate()` 및 `server/index.js` 두 부트 경로 모두에서 읽힌다. Config.Schema 의 `a2a` 객체 + Config.DEFAULTS 모두 등록됨. 코드는 옵셔널 체이닝(`config.a2a?.recoverOnStart !== false`) 을 legacy config 안전망으로 유지. |
